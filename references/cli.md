...
| 用户说 | agent 到点执行 |
|--------|----------------|
| 「每天 9/13/17 点跑一遍、打招呼到上限、收简历和沟通各 50」 | 三个时刻各跑：`python scripts/boss_pipeline.py --greet 999 --collect 50 --chat 50` |
| 「每天上午只跑打招呼到上限」 | `python scripts/cua_greeting_loop.py --limit 999` |
| 「每天早 6 点提醒今天面试」 | `python scripts/interview_reminder.py --within 1 --notify` |
| 「每周一同步岗位」 | `python scripts/cua_sync_jobs.py --write` |

映射要点：
- **「打招呼到上限」= `--greet`/`--limit` 设大数（如 999）**：撞到 BOSS 每日上限弹窗会**自动停**，不会硬刚。
