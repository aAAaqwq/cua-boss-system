...
| 10 | **设为定时任务**（可选） | **不用专门脚本**——由当前 agent 平台用它自己的定时机制，到点调用功能脚本 `boss_pipeline.py`。**默认节奏**：每天 9/13/17 点各跑一次 `python scripts/boss_pipeline.py --greet max --collect 50 --chat 50`（`--greet max`＝打招呼打到每日上限自动停，收简历/沟通各 50）。**前提**：到点电脑已登录且保持会话(防睡眠)、Chrome 已登录 BOSS、cua-driver 权限就绪、已 login（GUI 自动化要求）|
